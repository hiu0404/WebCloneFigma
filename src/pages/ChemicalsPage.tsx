...
export function ChemicalsPage() {
  ...
  const childCategories = useMemo(() => visibleChildCategories(categories, 'chemicals'), [categories])
  const selectedGroup =
  ...
  useEffect(() => {
    ...
    Promise.all([apiGetProducts(), apiGetCategories()])
      .then(([list, categoryList]) => {
        ...
        const visibleChemicals = categoryList.find(
          (category) => category.slug === 'chemicals' && isCategoryVisible(category),
        )
        setProducts(
          list
            .map(normalizeProductCategoryFields)
            .filter((product) => (product.parentCategorySlug ?? product.category ?? 'equipment') === 'chemicals')
            .filter((product) => (product.status ?? 'active') === 'active')
            .filter((product) => {
              if (!visibleChemicals) return false
              const childSlug = getProductChildCategorySlug(product)
              return visibleChemicals.children.some((child) => child.slug === childSlug && isCategoryVisible(child))
            }),
        )
      })
      .catch(() => {
  ...
  const selectedTitle =
    selectedGroup === 'all'
      ? 'Tất cả hóa chất và kháng thể'
      : getChildCategoryLabel(categories, 'chemicals', selectedGroup)
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.heroTitle}>
        <div>Hóa chất và kháng thể</div>
        <div className={styles.heroTitleSmall}>Trang chủ / Hóa chất và kháng thể</div>
      </div>
      ...
      <main className={styles.container}>
        ...
        <h1 className={styles.title}>Hóa chất và kháng thể</h1>

        <div className={styles.categorypage}>
          <aside className={styles.sidebar}>
            <h2>DANH MỤC HÓA CHẤT</h2>
            <ul>
              <li>
                <button
                  className={`${styles.categoryLink} ${selectedGroup === 'all' ? styles.active : ''}`}
                  type="button"
                  onClick={() => navigate('/ChemicalsPage')}
                >
                  <span>›</span>
                  Tất cả hóa chất ({products.length})
                </button>
              </li>
              {childCategories.map((item) => (
                <li key={item.slug}>
                  <button
                    ...
                    type="button"
                    onClick={() => navigate(`/ChemicalsPage/${item.slug}`)}
                  >
                    <span>›</span>
                    ...
                    <article className={styles.productcard} key={item.id}>
                      ...
                      <div className={styles.productcardContent}>
                        <span className={styles.groupPill}>
                          {getChildCategoryLabel(categories, 'chemicals', getProductChildCategorySlug(item))}
                        </span>
                        <h3>{item.title}</h3>
