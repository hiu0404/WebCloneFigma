...
export function AntibodiesPage() {
  const navigate = useNavigate()
  const { categorySlug } = useParams()
  const [products, setProducts] = useState<Product[]>([])
  const [categories, setCategories] = useState<AdminCategory[]>([])

  const childCategories = useMemo(() => visibleChildCategories(categories, 'antibodies'), [categories])
  const selectedGroup =
  ...
  useEffect(() => {
    ...
    Promise.all([apiGetProducts(), apiGetCategories()])
      .then(([list, categoryList]) => {
        ...
        const visibleAntibodies = categoryList.find(
          (category) => category.slug === 'antibodies' && isCategoryVisible(category),
        )
        setProducts(
          list
            .map(normalizeProductCategoryFields)
            .filter((product) => (product.parentCategorySlug ?? product.category ?? 'equipment') === 'antibodies')
            .filter((product) => (product.status ?? 'active') === 'active')
            .filter((product) => {
              if (!visibleAntibodies) return false
              const childSlug = getProductChildCategorySlug(product)
              return !childSlug || visibleAntibodies.children.some((child) => child.slug === childSlug && isCategoryVisible(child))
            }),
        )
      })
      .catch(() => {
  ...
  const selectedTitle =
    selectedGroup === 'all' ? 'Tất cả kháng thể' : getChildCategoryLabel(categories, 'antibodies', selectedGroup)
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.heroTitle}>
        <div>Kháng thể</div>
        <div className={styles.heroTitleSmall}>Trang chủ / Kháng thể</div>
      </div>
      ...
      <main className={styles.container}>
        ...
        <h1 className={styles.title}>Kháng thể</h1>

        <div className={styles.categorypage}>
          <aside className={styles.sidebar}>
            <h2>DANH MỤC KHÁNG THỂ</h2>
            <ul>
              <li>
                <button
                  className={`${styles.categoryLink} ${selectedGroup === 'all' ? styles.active : ''}`}
                  type="button"
                  onClick={() => navigate('/AntibodiesPage')}
                >
                  <span>›</span>
                  Tất cả kháng thể ({products.length})
                </button>
              </li>
              {childCategories.map((item) => (
                <li key={item.slug}>
                  <button
                    ...
                    type="button"
                    onClick={() => navigate(`/AntibodiesPage/${item.slug}`)}
                  >
                    <span>›</span>
                    ...
                    <article className={styles.productcard} key={item.id}>
                      ...
                      <div className={styles.productcardContent}>
                        <span className={styles.groupPill}>
                          {getChildCategoryLabel(categories, 'antibodies', getProductChildCategorySlug(item))}
                        </span>
                        <h3>{item.title}</h3>
